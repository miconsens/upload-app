...
const DownloadButton = ({
    ...
    <Mutation mutation={CREATE_PRESIGNED_LINK_MUTATION}>
    {
        (createPresignedLink, {called, data, error, loading}) => (
            !console.log(data) &&
            R.isNil(data) ?
            <Button floated ='right' size='mini' content='Create Presigned Link' onClick={() => createPresignedLink({variables: {bucketName, objectName}})}/>
            :
...
const UploadsList = ({
    ...
    <Query query={USER_UPLOADS} skip={R.isNil(user)} variables={{userID: R.prop('userID', user)}}>
    {
        ({data: {user}, error, loading, refetch}) => (
                <Segment basic>
                <Button inverted color='green' as={'label'} content='REFRESH' fluid  onClick={() => refetch()}/>
                {
                    !R.isNil(user) &&
                    R.compose(
                        R.map(
                            ({bucketName, objectName, filename}) => (
                                <Segment clearing key={objectName}>
                                    {filename}
                                    <DownloadButton bucketName={bucketName} objectName={objectName} filename={filename} />
                                </Segment>
                            )
                        ),            
                        R.prop('uploads')
...
const App = ({setPageKey, user, result, createUpload}) => {
    ...
    return (
        ...
        <Button
            size='massive'  style={{background:'#82E0AA', color:'#ffff'}} as={'label'} floated='right'
            animated='fade'
        ...
        {/* <Divider hidden vertical centered='true'> */}
                {<Button size='massive' animated='fade' style={{background:'#82E0AA', color:'#ffff'}} as={'label'}>
        ...
        {/* </Divider> */}
        <Segment>
            <Grid columns={3} style={{ height: '200vh' }} stackable textAlign='center'>
                <Grid.Row verticalAlign='top'>
                    <Grid.Column>
                        <Header style={{color:'#82E0AA'}} icon>
                            <br></br>
                            <Icon style={{color:'#82E0AA'}} name='file alternate outline' />
                            <br></br>
